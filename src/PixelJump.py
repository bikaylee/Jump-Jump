...
import numpy as np
from numpy.random import randint# Rllib docs: https://docs.ray.io/en/latest/rllib.html
...
class PixelJump(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        self.observation_space = Box(0, 1, shape=(
            np.prod([2, self.obs_size_z, self.obs_size_x]), ), dtype=np.int32)
    ...
    def step(self, action):
        ...
        self.velocity = self.velocity_min + \
            ((self.velocity_max-self.velocity_min) * action[0])
        ...
        movements = self.movement(
            self.velocity, self.XPos, self.YPos, self.ZPos, self.degree)  # self.degree)
